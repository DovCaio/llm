...
pdfs_urls = [
    ...
    "https://www.sda.ce.gov.br/wp-content/uploads/sites/60/2018/10/cartilha_cidadania_direito_ter_direitos.pdf?utm_source=chatgpt.com",
...
pages_urls = [
    'https://idec.org.br/noticia/em-decisao-atropelada-ans-tenta-aprovar-serie-de-medidas-contra-consumidores'
]
...
def dowload_pdf(url, save_path):
    ...
    print(f"Pdf baixado e salvo em {save_path}")
...
def clean_text_portuguese(raw_text):
    clean_text = re.sub(r"[^0-9a-zA-ZÀ-ÿ\s,.?!]", "", raw_text)

    clean_text = re.sub(r'\.{2,}', '.', clean_text)

    clean_text = re.sub(r'\b(\w+)(\s+\1\b)+', r'\1', clean_text, flags=re.IGNORECASE)

    return clean_text.replace("fig", "")

def process_text(text):
    text_without_links = re.sub(r'http\S+|www\.\S+', '', text)
    text_witout_blanks = re.sub(r'\s+', ' ', text_without_links).strip()
    clean_text = clean_text_portuguese(text_witout_blanks) #as vezes ele pega imagens que não são identificaveis pelo seu nome
